...
test('新版 Vue 页面保留核心筛选入口', () => {
  ...
  assert.match(appVue, /新用户转化率/);
  assert.match(appVue, /活跃用户关注私域占比/);
  assert.match(appVue, /导出当前报表/);
  assert.match(appVue, /data-dimension="日"/);
  assert.match(appVue, /data-dimension="周"/);
  assert.match(appVue, /data-dimension="月"/);
});

test('新版前端控制器保留关键业务动作和对比逻辑', () => {
  const controller = fs.readFileSync(path.resolve(__dirname, '../frontend/src/composables/useGdataDashboard.ts'), 'utf8');
  ...
  assert.match(controller, /function previousMonthChange/);
  assert.match(controller, /function comparisonCards/);
  assert.match(controller, /function syncPreviewText/);
});
